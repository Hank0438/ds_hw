...
Details *computerMove(int type, char* board[BOARD_SIZE][BOARD_SIZE]){
	Details *sth;
...
void readDetails(Details *first, char* tmpBoard[BOARD_SIZE][BOARD_SIZE]){
	...
	while(tmp != NULL){
		...
		if(type == 'A'){
			...
			for(u=s-2; u<s+3; u++){
				for(r=t-2; r<t+3; r++){
					if(tmpBoard[u][r] == NULL) tmpBoard[u][r] = " 1 ";
				}
			}
		}
		if(type == 'B'){
			...
			for(u=s-1; u<s+2; u++){
				for(r=t-1; r<t+2; r++){
					if(tmpBoard[u][r] == NULL) tmpBoard[u][r] = " 2 ";
				}
			}
		}
		if(type == 'C'){
...
//  如何把board清空，這樣就可以不用另外新增tmp	
//	clearBoard(board);
//	printBoard(board);
	
	
	char* tmp[BOARD_SIZE][BOARD_SIZE] = { NULL };
	readDetails(first, tmp);
	printBoard(tmp);
